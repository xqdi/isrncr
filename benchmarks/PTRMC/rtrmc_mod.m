...
% See also: buildproblem initialguess
    ...
    optproblem.cost = @(U, store) cost(U, store);
    optproblem.grad = @(U, store) gradient(U, store);


    % Specify whether to use the Hessian or not. If not, "approximate" it
    % with the identity matrix.
    if opts.order == 2
        optproblem.hess = @(U, H, store) hessian(U, H, store);
    elseif opts.order == 1
    ...
    if opts.computeRMSE
        opts.statsfun = @(optproblem, U, stats, store) recordrmse(optproblem, U, stats, store);
    end
    
    
    if ~isempty(problem_test)    
        opts.statsfun = @(problem, U, stats) mc_mystatsfun(problem, U, stats);
    end
