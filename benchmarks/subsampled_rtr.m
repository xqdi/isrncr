function [x, cost, info, options, best_x] = subsampled_rtr(problem, x, options)
...
%                 tCG(problem, x, fgradx, eta, Delta, options, storedb, key);
    ...
    if options.useExp  
        x_prop  = problem.M.exp(x, eta, 1);
    else
        x_prop  = problem.M.retr(x, eta, 1);
    end
...
%                       rhoden, accept, numit, norm_eta, used_cauchy);
    ...
    if options.verbosity == 2,
        ...
        'f: %+.8e    ft: %+.8e   |grad|: %e   %s\n'], ...
        mode, options.samp_hess_scheme, options.samp_grad_scheme, accstr,trstr,k,numit,fx, stats.cost_test,norm_grad,srstr);
    elseif options.verbosity > 2,
