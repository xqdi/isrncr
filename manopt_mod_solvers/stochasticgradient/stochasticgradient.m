function [x, info, options, best_x] = stochasticgradient(problem, x, options)
...
%      Added preconditioner support. This allows to use adaptive algorithms.
    ...
    savedstats = 0;
    [cost_, grad_] = getCostGrad(problem, x, storedb, key);
    ...
    if options.verbosity >= 2
        fprintf('    iter       time [s]       step size   cost\n');
    end
    ...
    while iter < options.maxiter
        ...
        storedb.purge();%storedb.removefirstifdifferent(key, newkey);
        ...
        if mod(iter, options.checkperiod) == 0
            ...
            % Print output.
            if options.verbosity >= 2
                fprintf('%8d     %10.2f       %.3e  %.6f\n', ...
                                               iter, stats.time, stepsize, cost_);
            end
