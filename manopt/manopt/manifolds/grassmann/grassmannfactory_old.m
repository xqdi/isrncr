function M = grassmannfactory_old(n, p, k)
...
%       (this only matters if k>1).
    ...
    function Y = exponential(X, U, t, k)
        if nargin == 3
            tU = t*U;
        else
            tU = U;
    ...
    M.exp = @(X, U, t) exponential(X, U, t, k);
    ...
    function U = logarithm(X, Y, n, p, k)
        U = zeros(n, p, k);
        for i = 1 : k
            x = X(:, :, i);
            y = Y(:, :, i);
    ...
    M.log = @(X, Y) logarithm(X, Y, n, p, k);
    
    M.hash = @(X) ['z' hashmd5(X(:))];
    
    
    function X = random(n, p, k)
        X = zeros(n, p, k);
        for i = 1 : k
    ...
    function U = randomvec(X, n, p, k)
        U = projection(X, randn(n, p, k));
        U = U / norm(U(:));
    end
    M.randvec = @(X) randomvec(X, n, p, k);
