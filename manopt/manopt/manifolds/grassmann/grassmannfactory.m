...
%       Here, 'ambient' refers to the total space.
    ...
    M.ehess2rhess = @(X, egrad, ehess, H) ehess2rhess(X, egrad, ehess, H);

    function Y = retraction(X, U, t, k)
        Y = X + t*U;
        for kk = 1 : k
    ...
    M.retr = @(X, U, t) retraction(X, U, t, k);
    ...
    M.invretr = @(X, Y) invretr(X, Y);

    % See Eq. (2.65) in Edelman, Arias and Smith 1998.
    function Y = exponential(X, U, t, k, array_type)
        tU = t*U;
    ...
    M.exp = @(X, U, t) exponential(X, U, t, k, array_type);
    ...
    function U = logarithm(X, Y, n, p, k, array_type)
        U = zeros(n, p, k, array_type);
        for kk = 1 : k
            x = X(:, :, kk);
            y = Y(:, :, kk);
    ...
    M.log = @(X, Y) logarithm(X, Y, n, p, k, array_type);

    M.hash = @(X) ['z' hashmd5(X(:))];
    
    function X = random(n, p, k, array_type)
    ...
    M.pairmean = @(X1, X2) pairmean(X1, X2);
