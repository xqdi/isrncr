function [eta, Heta, hesscalls, stop_str, stats] = lanczos_solver(problem, x, grad, gradnorm, sigma, samp_hess_idx, samp_hess_p, options, storedb, key)
% Subproblem solver for ARC based on a Lanczos process.
%
% [eta, Heta, hesscalls, stop_str, stats] = 
%     lanczos_solver(problem, x, grad, gradnorm, sigma, options, storedb, key)
%
...
% Parlett), for possible numerical fixes.
    ...
    if samp_hess_size >= problem.ncostterms
      %Hess = @(u) getHessian(problem, x, u, storedb, key);
      Hess = @(u) getPartialHessian(problem, x, u, samp_hess_idx, samp_hess_p, storedb, key); 
    else
      Hess = @(u) getPartialHessian(problem, x, u, samp_hess_idx, samp_hess_p, storedb, key); 
    end
    ...
    for j = 1 : min(options.maxinner, n) - 1
        ...
        if model_gradnorm <= options.theta*norm(y)^2 %options.kappa * min(1, norm(y)) * gradnorm;
    ...
    points = numel(y);
    if ~gradnorm_reached
        stop_str = 4;%sprintf(['Reached max number of Lanczos iterations ' ...
